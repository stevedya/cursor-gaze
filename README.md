...
Open the local URL printed by Vite. Camera access works on `localhost` or HTTPS. Choose **7 × 7** for a quicker capture or **13 × 13** for twice as many steps along each axis. Under **Output detail**, choose **Standard** for a smaller, mobile-friendly sheet or **High detail** for a larger desktop-focused sheet. Then click **Enable camera** in the preview or control panel and accept the browser prompt. If permission was previously blocked, open the site controls beside the address bar, set Camera to Allow, and refresh. Sit centered with some space around your head, then press **Start capture** (or Space). After a three-second countdown, follow the moving dot with your head and eyes. The path begins at the center of the camera preview and spirals outward, pausing at every target before taking a frame.

Each frame is saved automatically to IndexedDB in the current browser. Each grid size and detail setting has its own saved session, so switching settings keeps existing frames. The selected setting is remembered after refresh. Escape or **Pause and keep frames** stops the run without deleting photos. Refreshing restores a complete capture, or lets you resume at the remaining positions. **Delete saved session** asks for confirmation before clearing only the selected session's frames. Browser storage is local to that browser and site; clearing site data or using another browser removes access to it. Downloaded files are the durable backup.
...
Capture timing, grid sizes, output frame dimensions, and WebP quality live in [`src/capture/config.ts`](src/capture/config.ts). Standard 7 × 7 uses 500 px frames for a 3500 × 3500 sheet; high detail uses 640 px frames for 4480 × 4480. Standard 13 × 13 uses 300 px frames for a 3900 × 3900 sheet; high detail uses 360 px frames for 4680 × 4680. The exporter uses 0.96 WebP quality for either setting. These larger sheets may exceed the canvas limit on iOS, so capture high-detail sessions in a desktop browser and test the finished portfolio on mobile. The denser capture takes roughly three minutes and reduces visible jumps between directions, though it still switches between discrete frames. The camera requests roughly 1920 × 1080 and falls back to its default mode when needed. High detail needs a new capture to gain actual detail; enlarging an older sprite does not recover it.
...
Choose **Sample** for the included illustrated 7 × 7 sprite, a **Saved** button for any complete capture stored in this browser, or **My files** to select your own sprite sheet and manifest. High-detail saved captures are marked **HD**. The uploader checks that the sprite dimensions match the manifest, then renders it with the same `CursorPortrait` component used on the production site. Uploaded files stay local to the browser tab and need to be selected again after a refresh.
